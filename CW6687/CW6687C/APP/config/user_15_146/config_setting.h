...
#define TASK_MUSIC_EN                   0   //是否打开MUSIC功能
#define TASK_AUX_EN                     0   //是否打开AUX功能
...
#define TASK_UPDATE_FW_EN               0   //是否打开离线升级模式（U盘或者TF卡均可）
...
#define STANDBY_TIME                	10 	//进入休眠待机(HOLD模式)时间(单位为秒)
#define HOLD_POWER_TYPE                 1   //HOLD MODE下功耗选择 0:正常电压，1:低电压
...
#define SOFT_POWER_OFF_20UA             0   //20uA软关机
#define SOFT_POWER_OFF_3UA              1   //3uA软关机 （CW6687E与CW6687G不支持）
...
#define WARNING_VOLUME                  5  //提示音的音量
...
#define FM_CLK_SEL                      P33_OUT_12M     //FM时钟选则
...
#define LINEIN_DETECT_EN                0                   //是否使用LINEIN检测
...
#define BT_ANA_GAIN                     7     //MIC模拟增益,范围为-1~9
...
#define BT_DYNAMIC_NOISE                DYNAMIC_DAC_ONLY     //蓝牙播放是否使用动态降噪
#define BT_ANTI_LOST                    0     //是否打开蓝牙防丢功能
...
#define AUX_DECT_REUSE_ADKEY            0       //是否使用ADKEY检测AUX
...
#define VCMBUF_EN                       1       //0不使用VCMBUF,1 使用VCMBUF直推耳机 (VCMBUF与P01绑在一起，使用VCMBUF时需要关闭P01打印的两个宏DEBUG_LOADER_DIS->1, DEBUG_UART_SEL->0)
...
#define MICBIAS_EN                      1       //是否使用MICBIAS给MIC供电，0：为使用VDDHP给MIC供电

#define SMALL_VOLUME_EN                 1       //是否使用小音量表(1.96v), 0为大音量表(2.8v)
#define INTERNAL_CHARGE_EN              1       //是否使用内部充电功能（CW6687E与CW6687G不支持）
#define LED_POWER_BLINK                 1       //是否使用充电指示灯
...
#define USE_ADKEY                       0           //ADKey的使用，0为不使用，1为1组ADKey，2为2组ADKey
#define USE_IOKEY                       1           //ADKey的使用，0为不使用，1为使用IOKey
...
#define ADKEY_INT_PU10K                 1           //第1组ADKEY是否使用内部10K上拉（只有3个按键时可以使用）
#define ADKEY2_INT_PU10K                0           //第2组ADKEY是否使用内部10K上拉（只有3个按键时可以使用）

#define K_PLAY_INIT()
#define IS_K_PLAY()                     (0)
#define K_NEXT_INIT()                   P1DIR |= BIT(3);P1PU |= BIT(3);
#define IS_K_NEXT()                     (!(P1 & BIT(3)))
#define K_PREV_INIT()                   P3DIR |= BIT(3);P3PU |= BIT(3);
#define IS_K_PREV()                     (!(P3 & BIT(3)))
...
//蓝色LED
#define LED_BLUE                        2
#define LED_BLUE_INIT()                 P3PU &= ~BIT(LED_BLUE); P3DIR |= BIT(LED_BLUE);  P3PUS0 |= BIT(LED_BLUE);P3PUS1 &= ~BIT(LED_BLUE)
#define LED_BLUE_ON()                   P3PU |= BIT(LED_BLUE)
#define LED_BLUE_OFF()                  P3PU &= ~BIT(LED_BLUE)
#define LED_BLUE_TOG()                  P3PU ^= BIT(LED_BLUE)
#define LED_BLUE_IS_ON()                (P3PU & BIT(LED_BLUE))
...
#define LED_POWER_IS_ON()               (P3PU & BIT(LED_POWER))
