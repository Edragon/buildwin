...
#define TASK_RTC_EN                     0   //是否打开时钟功能
#define TASK_SPEAKER_EN                 0   //是否打开扩音器模块
#define TASK_MIC_EN                     0   //是否打开MIC录音模块
#define TASK_VOICE_EN                   0   //是否打开录音播放模块
...
#define TASK_BTHID_EN                   0   //是否打开独立自拍器模式
...
#define STANDBY_TIME                	15  //进入休眠待机(HOLD模式)时间(单位为秒)
...
#define AUX_AUTO_STANDBY_EN             0   //AUX/AUX EQ任务下，是否需要自动进入休眠待机
...
#define FM_CLK_SEL                      FMOSC_REUSE_12MSDCLK     //FM时钟选则
...
#define FM_CHANNEL_SEL                  CHANNEL_D01         //定制FM的通道
#define AUX_CHANNEL_SEL                 CHANNEL_D21       	//选择LINEIN的通道
...
#define AUX_RECORD_EN                   0                   //是否打开AUX录音功能
...
#define BT_SPP_MSC_EN                   0     //是否使用SPP获取MUSIC曲目及播放列表（需同时打开BT_ALL_TASK及BT_SPP_EN宏定义）
#define BT_SPP_EQ_EN                    0     //是否打开蓝牙在线调试EQ功能（需同时打开BT_ALL_TASK及BT_SPP_EN宏定义）
#define BT_HID_EN                       0     //是否打开蓝牙HID（自拍功能）服务
...
#define BT_ANA_GAIN                     5     //MIC模拟增益,范围为-1~9
...
#define ALARM_EN                        0       //闹钟
#define ALARM_WKUP_POWERDOWN            0       //power down关机后，闹钟可以唤醒开机
...
#define EAR_DETECT_EN                   0       //是否使用耳机检测（耳机插入时MUTE功放）
#define EAR_SEG_REUSE                   0       //耳机检测复用LED SEG
#define EAR_ADKEY_REUSE                 0       //耳机检测复用ADKEY
...
#define LOWER_POWER_DISPLAY_OFF         0       //低电时是否需要显示一下OFF并且在3秒后关机
#define LED_STATUS_BLINK                0       //是否使用电池电量状态灯（低电时闪烁）
#define MODE_CHANGE_BLUE_ON             1       //是否需要模式切换时蓝灯闪烁一下
...
//LDO MODE下的PMU配置
#define USER_OTHER_INIT()               ssop28_pmu_init()

/*****************************************************************************
 * Module    : 按键及外设IO配置
 *****************************************************************************/

#define USE_ADKEY                       1           //ADKey的使用，0为不使用，1为1组ADKey，2为2组ADKey
#define USE_IOKEY                       0           //IOKey的使用，0为不使用，1为使用IOKey
#define USE_WKOKEY                      0           //1为检测WKO PIN上的按键，0不用此功能
#define KEY_DOUBLE_CLICK_EN             1           //使能某一个按键的双击功能,按键在宏DOUBLE_CLICK_KEY中配置
#define DOUBLE_CLICK_KEY                KU_PLAY     //支持双击的按键
#define LONG_PRESS_POWERON              0           //长按某键才能开机。
...
#define SD_CMD_REUSE_SD_DETECT          1
...
#define OTHER_ADC_CHECK()               other_adc_check()	//其它ADKE1的检测，如AUX/SD/耳机检测复用ADKEY1
#define OTHER_ADC2_CHECK()                          		//其它ADKE2的检测，如AUX/SD/耳机检测复用ADKEY2

/*****************************************************************************
 * Module    : IIC的相关配置
 *****************************************************************************/
#if (FM_CLK_SEL == P33_OUT_12M)
//--------------------------------------------------------------------------
#define IIC_IO_SET
...
#define IIC_DATA                        2      //P32
#define IIC_CLK                         1      //P31
#define CLK_PU500()                     P3PD &=~BIT(IIC_CLK);  P3PU |= BIT(IIC_CLK);  P3PUS1 &= ~BIT(IIC_CLK);  P3PUS0 |= BIT(IIC_CLK);    P3DIR |= BIT(IIC_CLK);  /*关下拉，开500上拉*/
#define CLK_PD500()                     P3PU &=~BIT(IIC_CLK);  P3PD |= BIT(IIC_CLK);  P3PDS1 &= ~BIT(IIC_CLK);  P3PDS0 |= BIT(IIC_CLK);    P3DIR |= BIT(IIC_CLK);  /*关上拉，开500下拉*/
#define DAT_PU500()                     P3PD &=~BIT(IIC_DATA); P3PU |= BIT(IIC_DATA); P3PUS1 &= ~BIT(IIC_DATA); P3PUS0 |= BIT(IIC_DATA);   P3DIR |= BIT(IIC_DATA); /*关下拉，开500上拉*/
#define DAT_PU10K()                     P3PD &=~BIT(IIC_DATA); P3PU |= BIT(IIC_DATA); P3PUS1 &= ~BIT(IIC_DATA); P3PUS0 &= ~BIT(IIC_DATA);  P3DIR |= BIT(IIC_DATA); /*关下拉，开10K上拉*/
#define DAT_PD500()                     P3PU &=~BIT(IIC_DATA); P3PD |= BIT(IIC_DATA); P3PDS1 &= ~BIT(IIC_DATA); P3PDS0 |= BIT(IIC_DATA);   P3DIR |= BIT(IIC_DATA); /*关上拉，开500下拉*/

#define IIC_DATA_CLK_OUT()              CLK_PU500(); DAT_PU500();delay(10)
#define IIC_DATA_IN()                   DAT_PU10K()
#define IIC_DATA_OUT()                  DAT_PU500()
#define IIC_DATA_CLK_HIGH()             CLK_PU500();DAT_PU500()
#define IIC_DATA_HIGH()                 DAT_PU500()
#define IIC_DATA_LOW()                  DAT_PD500()
#define IIC_CLK_HIGH()                  CLK_PU500()
#define IIC_CLK_LOW()                   CLK_PD500()
#define IIC_DATA_IS_HIGH()              (P3 & BIT(IIC_DATA))
//--------------------------------------------------------------------------
#endif
...
//蓝色LED
#define LED_BLUE                        0
#define LED_BLUE_INIT()                 P2PU &= ~BIT(LED_BLUE); P2DIR |= BIT(LED_BLUE);  P2PUS0 |= BIT(LED_BLUE);P2PUS1 &= ~BIT(LED_BLUE);
#define LED_BLUE_ON()                   P2PU |= BIT(LED_BLUE);
#define LED_BLUE_OFF()                  P2PU &= ~BIT(LED_BLUE)
#define LED_BLUE_TOG()                  P2PU ^= BIT(LED_BLUE)
#define LED_BLUE_IS_ON()                (P2PU & BIT(LED_BLUE))
 ...
 *****************************************************************************/
//MUTE功放控制
#define SPEAKER_MUTE()                  P0DIR &= ~BIT(1); P0 |= BIT(1)
#define SPEAKER_UNMUTE()                P0DIR &= ~BIT(1); P0 &= ~BIT(1)
...
#define DCIN_INIT()                     //P1DIR |= BIT(7); //外部有下拉,这里就不开内部下拉了
#define IS_DCIN()                       0//(P1 & BIT(7))

#define MP3BT_DECT_INIT()               P3DIR |= BIT(0)          //上电时BT/MP3检测
#define MP3BT_DECT_IS_BT()              (!(P3 & BIT(0)))  		 //接地为BT
#define MP3BT_DECT_END()                P3DIR &= ~BIT(0)         //复用时要自定义
