...
{
    //printf("spiwr: %04x\n", spimsc_ctl.cur_addr);
    if ((spimsc_ctl.cur_addr & SPI_ERASE_BLOCK_MASK) == 0) {
    ...
    if (spimsc_ctl.data_len >= spimsc_ctl.end_addr) {
        ...
        spimsc_ctl.data_len -= SPI_ERASE_BLOCK;
        spimsc_ctl.start_addr += SPI_ERASE_BLOCK;
       // printf("len:%d\n",spimsc_ctl.data_len);
...
//对齐当前位置，设为新的录音起始位置
#pragma location="TASK_TAPE_INIT"
void spi_record_start(void)
{
    u16 repeat_addr;
    repeat_addr = ((spimsc_ctl.cur_addr + SPI_ERASE_BLOCK_MASK) >> SPI_ERASE_BLOCK_BIT) << SPI_ERASE_BLOCK_BIT;
