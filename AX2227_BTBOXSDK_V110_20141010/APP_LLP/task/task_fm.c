...
extern void task_fm_record_display(void);       //录音状态下的显示
...
void task_fm_mp3en_init(void)
{
    mp3en_exit();                   //关闭AMUX模块
    mp3en_source_init(RECORD_FM);   //初始化encoder模块
...
//FM录音
#pragma location="TASK_FM_INIT"
void task_fm_record(void)
{
    task_fm_reuse_exit();
    task_record(RECORD_FM);
    task_fm_reuse_init();
...
#if (FM_CHANNEL_SEL & 0x07 == CHANNEL_1)
#if DEBUG_UART_SEL
    uart_disable();
#endif
    P0DIR |= BIT(1);
...
#if DEBUG_UART_SEL && (FM_CHANNEL_SEL & 0x07 == CHANNEL_1)
    P0DIR &= ~BIT(1);
    uart_enable();
#endif
